...
from utils.losses import QuantileLoss, SPECTRALoss
import torch
...
class Exp_Forecast(Exp_Basic):
    ...
    def _select_criterion(self):
        ...
        elif self.args.loss_type == 'quantileLoss':
            criterion = QuantileLoss(self.args.quantiles)
        elif self.args.loss_type == 'squantileLoss':
            criterion = SPECTRALoss(self.args.quantiles, self.args.loss_alpha, self.args.top_k)
        else:
            criterion = nn.MSELoss()
        return criterion
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if i % 20 == 0:
                    ...
                    # pred 输出为「B Q L D」Q 为分位数输出个数，pred[:, pred.shape[1] // 2, :, -1] 代表中位数预测结果
                    pd = np.concatenate((input[0, :, -1], pred[0, pred.shape[1] // 2, :, -1]), axis=0)

...
class TFTEmbedding(nn.Module):
    def __init__(self, configs):
        ...
        self.static_pos = datatype_dict[configs.data_path[:-4]].static
        self.observed_pos = datatype_dict[configs.data_path[:-4]].observed
...
class TemporalFusionDecoder(nn.Module):
    def __init__(self, configs):
        ...
        self.out_projection = nn.Linear(configs.d_model, configs.c_out * self.num_quantiles)
    ...
    def forward(self, history_input, future_input, c_c, c_h, c_e):
        ...
        bsz, out_len, _ = out.shape
        out = out.reshape(bsz, out_len, self.num_quantiles, self.c_out)
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # Number of variables
        self.static_len = len(datatype_dict[configs.data_path[:-4]].static)
        self.observed_len = len(datatype_dict[configs.data_path[:-4]].observed)

...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.projection = nn.Linear(
            configs.d_model, self.c_out * self.num_quantiles, bias=True)
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        B, T, _ = dec_out.shape
        dec_out = dec_out.view(B, T, self.num_quantiles, self.c_out)
        dec_out = dec_out.permute(0, 2, 1, 3)  # [B, Q, L, V]
        ...
        scale = stdev[:, 0, :self.c_out].unsqueeze(1).unsqueeze(1)
        shift = means[:, 0, :self.c_out].unsqueeze(1).unsqueeze(1)
        dec_out = dec_out * scale
        dec_out = dec_out + shift

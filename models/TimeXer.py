...
class Model(nn.Module):
    ...
    def get_parameter_number(self):
        ...
        trainable_ratio = sum(p.numel() for p in self.parameters() if p.requires_grad) / total_num
        print(f'Model Name: SPECTRA Architecture')
        print(f'Total Params: {total_num} | Memory: {param_memory:.2f} MB | Trainable Ratio: {trainable_ratio:.2%}')
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        if self.use_norm:
            ...
            dec_out = dec_out * (stdev[:, 0, -1:].unsqueeze(1).unsqueeze(1))
            dec_out = dec_out + (means[:, 0, -1:].unsqueeze(1).unsqueeze(1))
